// ============================================================
// SE HM Progression — Personal View Logic
// ============================================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (playerId && PLAYERS[playerId]) {
    ...
    highlightStyle.innerHTML = `.owner-pill[data-owner-id="${playerId}"] { background-color: var(--accent); color: var(--bg-main); border-color: var(--accent); }`;
...
function populateDropdown(currentId) {
  ...
  if (currentId && PLAYERS[currentId]) selector.value = currentId;
  selector.addEventListener('change', (e) => { if (e.target.value) window.location.search = `?id=${e.target.value}`; });
}
...
function renderContent(id, assignmentRenderer) {
  ...
  FIGHTS.forEach(fight => {
    const card = document.createElement('div'); card.className = 'personal-fight-card';
    ...
    const mentions = [];
    const myBuffs = (fight.buffsDebuffs || []).filter(bd => bd.owners.includes(id)).map(bd => bd.name);
    ...
    if (fight.strategy.phases) {
      fight.strategy.phases.forEach(phase => {
        ...
        if (phaseMentions.length) { mentions.push(`<strong>${phase.name}:</strong>`); mentions.push(...phaseMentions); }
      });
    }
    card.innerHTML = `
      <div class="personal-fight-header"><span style="font-size:1.25rem">${fight.icon}</span><h2>${fight.name}</h2></div>
      <div class="personal-fight-body"><div class="fight-content-grid">
        <div class="fight-col-build"><div class="personal-section" style="margin-bottom:0"><h3>🛡️ Build</h3>${buildHtml}</div></div>
        <div class="personal-section"><h3>📋 Assignments</h3>
        ...
        <div class="personal-section"><h3>⚙️ Strategy Notes</h3>
...
function findMentions(lines, id) { return lines.filter(line => line.includes(id)); }
