...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const playerOrder = [MAIN_ROLE_ID.MT, MAIN_ROLE_ID.OT, MAIN_ROLE_ID.H1, MAIN_ROLE_ID.H2,
    MAIN_ROLE_ID.DPS1, MAIN_ROLE_ID.DPS2, MAIN_ROLE_ID.DPS3, MAIN_ROLE_ID.DPS4,
    MAIN_ROLE_ID.DPS5, MAIN_ROLE_ID.DPS6, MAIN_ROLE_ID.DPS7, MAIN_ROLE_ID.DPS8];
  playerOrder.forEach(id => {
    ...
    var tag_render = "";
    if (PLAYERS[id].tag) tag_render = ` [${PLAYERS[id].tag}]`;
  ...
  if (urlTab && ['builds', 'assignments', 'strategy', 'buffs'].includes(urlTab)) currentTab = urlTab;
  ...
  if (btnPersonal) {
    btnPersonal.addEventListener('click', () => {
      ...
      if (pid && pid !== 'all') window.open(`player.html?id=${pid}`, '_blank');
      else alert('Please select a specific player from the dropdown first.');
    });
  }
  ...
  modalStyle.innerHTML = `
    .modal-overlay { position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;z-index:1000;cursor:pointer; }
    .modal-content { position:relative;background:var(--bg-main);padding:20px;border-radius:8px;max-width:90vw;max-height:90vh;cursor:default; }
    .modal-content img { display:block;max-width:100%;max-height:calc(90vh - 40px);border-radius:4px; }
    .modal-close { position:absolute;top:-10px;right:-10px;width:30px;height:30px;background:var(--accent);color:var(--bg-main);border-radius:50%;border:2px solid var(--bg-main);display:flex;align-items:center;justify-content:center;font-weight:bold;cursor:pointer;font-size:1.2rem;line-height:1; }
  `;
  ...
  document.body.addEventListener('click', (e) => {
    if (e.target.classList.contains('show-map-btn')) {
      ...
      overlay.innerHTML = `<div class="modal-content"><img src="${imgSrc}" alt="Map"><div class="modal-close">&times;</div></div>`;
      document.body.appendChild(overlay);
      const closeModal = () => { if (document.body.contains(overlay)) document.body.removeChild(overlay); };
      overlay.addEventListener('click', (ev) => { if (ev.target === overlay || ev.target.classList.contains('modal-close')) closeModal(); });
    }
  });
});
...
function selectFight(fightId) {
  ...
  document.querySelectorAll('.fight-sel-btn').forEach(b => b.classList.toggle('active', b.dataset.fight === fightId));
  document.querySelectorAll('.nav-fights a[data-fight]').forEach(a => a.classList.toggle('active', a.dataset.fight === fightId));
  ...
  buildsPanel.innerHTML = ''; assignPanel.innerHTML = ''; stratPanel.innerHTML = ''; buffsPanel.innerHTML = '';
  ...
  if (fight.buffsDebuffs && fight.buffsDebuffs.length) buffsPanel.appendChild(renderBuffsTable(fight.buffsDebuffs));
  else buffsPanel.innerHTML = '<p style="color:var(--text-muted);font-size:0.9rem;">No buff/debuff data for this fight.</p>';
...
function switchTab(tabId) {
  currentTab = tabId;
  updateURL();
  document.querySelectorAll('#fight-tab-bar .tab-btn').forEach(b => b.classList.toggle('active', b.dataset.tab === tabId));
  ['builds', 'assignments', 'strategy', 'buffs'].forEach(id => document.getElementById(`panel-${id}`).classList.toggle('active', id === tabId));
}
...
function renderBuilds(fight) {
  ...
  playerOrder.forEach(pid => {
    ...
    const skillsHtml = skills.length ? `<div class="build-skills">${skills.map(s => `<span class="skill-pill">${s.skill}</span>`).join('')}</div>` : '';
    const miscRowHTML = b.misc && b.misc.length ? `<div class="build-row">${b.misc.map(s => `<span class="misc-pill">${s}</span>`).join('')}</div>` : '';
    card.innerHTML = `
      ...
      <div class="build-row">${b.sets.map(s => `<span class="set-pill">${s}</span>`).join('')}</div>
      <div class="build-row"><span class="ult-pill">Ult: ${b.ult}</span></div>
      ${miscRowHTML}${skillsHtml}
...
function renderStrategy(fight) {
  ...
  if (fight.strategy.details && fight.strategy.details.length) {
    ...
    fight.strategy.details.forEach(d => { const li = document.createElement('li'); li.innerHTML = resolvePlayerNameAsPill(d); ul.appendChild(li); });
  ...
  if (fight.strategy.phases) {
    fight.strategy.phases.forEach(phase => {
      ...
      const header = document.createElement('div'); header.className = 'phase-header'; header.textContent = phase.name;
      header.addEventListener('click', () => { header.classList.toggle('collapsed'); phaseBody.classList.toggle('collapsed'); });
      block.appendChild(header);
      const phaseBody = document.createElement('div'); phaseBody.className = 'phase-body';
      const ul = document.createElement('ul');
      phase.details.forEach(d => { const li = document.createElement('li'); li.innerHTML = resolvePlayerNameAsPill(d); ul.appendChild(li); });
      phaseBody.appendChild(ul); block.appendChild(phaseBody); container.appendChild(block);
    });
  }
  if (fight.strategy.reminders && fight.strategy.reminders.length) {
    const rem = document.createElement('div'); rem.className = 'reminders'; rem.innerHTML = '<h4>⚠ Reminders</h4>';
    const ul = document.createElement('ul'); ul.style.listStyle = 'none';
    fight.strategy.reminders.forEach(r => { const li = document.createElement('li'); li.innerHTML = resolvePlayerNameAsPill(r); ul.appendChild(li); });
    rem.appendChild(ul); container.appendChild(rem);
  }
  if (fight.strategy.supplemental && fight.strategy.supplemental.length) {
    const supp = document.createElement('div'); supp.className = 'supplemental-section';
    supp.style.marginTop = '1.5rem'; supp.style.borderTop = '1px solid var(--border)'; supp.style.paddingTop = '1rem';
    fight.strategy.supplemental.forEach(s => {
      const h4 = document.createElement('h4'); h4.textContent = s.title;
      h4.style.color = 'var(--text-muted)'; h4.style.fontSize = '0.9rem'; h4.style.textTransform = 'uppercase'; h4.style.marginBottom = '0.5rem';
      supp.appendChild(h4);
      const ul = document.createElement('ul');
      s.content.forEach(line => { const li = document.createElement('li'); li.innerHTML = resolvePlayerNameAsPill(line); ul.appendChild(li); });
      supp.appendChild(ul);
    });
...
function sortBuffsDebuffs(list) {
  return [...list].sort((a, b) => {
    const aHas = a.owners.length ? 0 : 1; const bHas = b.owners.length ? 0 : 1;
    if (aHas !== bHas) return aHas - bHas;
    return a.name.localeCompare(b.name);
  });
}
function buildMiniTable(items, label) {
  const col = document.createElement('div'); col.className = 'buffs-col';
  col.innerHTML = `<h5 class="buffs-col-label">${label}</h5>`;
  const table = document.createElement('table'); table.className = 'buffs-table';
  table.innerHTML = '<thead><tr><th>Modifier</th><th>Responsible</th></tr></thead>';
  const tbody = document.createElement('tbody');
  items.forEach(bd => {
    const tr = document.createElement('tr'); if (!bd.owners.length) tr.className = 'unassigned';
    const tdName = document.createElement('td'); tdName.textContent = bd.name;
    const tdOwner = document.createElement('td');
    if (bd.owners.length) tdOwner.innerHTML = bd.owners.map(id => createOwnerPillHtml(id)).join(' ');
    else tdOwner.innerHTML = '<span class="no-owner">—</span>';
    tr.appendChild(tdName); tr.appendChild(tdOwner); tbody.appendChild(tr);
  });
  table.appendChild(tbody); col.appendChild(table); return col;
}
function renderBuffsTable(buffsDebuffs) {
  const wrapper = document.createElement('div'); wrapper.className = 'buffs-table-wrapper standalone';
  ...
  const row = document.createElement('div'); row.className = 'buffs-two-col';
  row.appendChild(buildMiniTable(buffs, '✦ Buffs'));
  row.appendChild(buildMiniTable(debuffs, '✧ Debuffs'));
  wrapper.appendChild(row); return wrapper;
}
...
function renderGuidance() {
  ...
  const grid = document.createElement('div'); grid.className = 'guidance-grid';
  const tankCard = document.createElement('div'); tankCard.className = 'guidance-card guidance-tank';
  tankCard.innerHTML = `<h3>Tanks</h3>
  ...
  const dpsCard = document.createElement('div'); dpsCard.className = 'guidance-card guidance-dps';
  dpsCard.innerHTML = `<h3>DPS</h3>
...
function renderReferences() {
  ...
  if (REFERENCES.videos && Object.keys(REFERENCES.videos).length) {
    const vidCard = document.createElement('div'); vidCard.className = 'ref-card';
    vidCard.innerHTML = '<h3>🎬 Video Guides</h3>';
    Object.entries(REFERENCES.videos).forEach(([key, url]) => {
      const a = document.createElement('a'); a.href = url; a.target = '_blank'; a.textContent = key;
      vidCard.appendChild(a);
    });
    grid.appendChild(vidCard);
  }

  if (REFERENCES.penCrit && Object.keys(REFERENCES.penCrit).length) {
    const penCard = document.createElement('div'); penCard.className = 'ref-card';
    penCard.innerHTML = '<h3>🔢 Pen/Crit Calculators</h3>';
    Object.entries(REFERENCES.penCrit).forEach(([key, url]) => {
      const a = document.createElement('a'); a.href = url; a.target = '_blank'; a.textContent = key;
      penCard.appendChild(a);
    });
